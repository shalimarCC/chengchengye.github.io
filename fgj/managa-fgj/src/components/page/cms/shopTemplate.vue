    ...
    <el-table :key='tableKey' :data="dataSource" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="所属客户端" min-width="80">
            <template slot-scope="scope">
                ...
                <span v-else-if="scope.row.clientType==4">web商城</span>
            </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作" width="350">
            <template slot-scope="scope">
                ...
                <el-button type="primary" size="small" @click.native="seeTemplate(scope.row.templateId)">预览</el-button>
            </template>
        </el-table-column>
...
export default {
    ...
    methods: {
        ...
        async addShopTemplate(){
            try {
                const res = await createEmptyTmpl({type:0,companyType:this.ruleForm.companyType});
        ...
        //预览
        seeTemplate(id){
            this.$router.push('/shopTemplateView/'+id);
        },
        //编辑
        editTemplate(id){
