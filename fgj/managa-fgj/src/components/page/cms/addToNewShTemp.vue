            ...
            <el-row :gutter="60">
                <el-col v-if="form.elemForm" v-for="(infos,key) in form.elemForm" :key="key">
                    <div v-if="infos.config" v-for="(info,key) in infos.config">
                        ...
                        <el-form-item label="配置参数" prop="name" v-if="form.linkType==8 && info.type == 0">
                            <el-select v-model="form.linkId">
                                <el-option
...
import {findStoreByPlatformId,findCommodityProductUnitLimit,findCouponBatchOfPage,findCategoryTreeAll,findStoreAllInfo,findCmsLocalParamAll,findCmsTemplateOfPage,findCmsTemplateAll,findCmsPageAllByClientType,findByStandardUnitName,findSimplePageTabAll,findCompanyIdOrName,findExternalLinkAll,queryStandardUnitCombinationAll,localParamList,getQiNiuToken,findCfgKeyByTemplateId,queryBannerList,insertCmsPageWithTx} from '../../../api/getData';
export default{
    data(){
        return{
            form:{
    ...
    methods: {
        ...
        async findCfgKeyByTemplateId(id){
            try {
                ...
                if (res.data.code == 0) {
                    this.form.tempForm = res.data.data.templateConfig;
                    this.form.elemForm = res.data.data.elementConfig;
                    $.each(res.data.data.templateConfig,function(key,info){
                        if(info.defaultValue && info.defaultValue !=null){
                            info.value = info.defaultValue;
                        }
                    })
                    console.log(this.form.tempForm);
                    console.log(this.form.elemForm);
                } else {
                    this.$message({
                        type: 'error',
                        message: res.data.error
        ...
        saveTemplate(){
            ...
            if(that.form.name.length>30){
                ...
                that.insertCmsPageWithTx (saveObj);
            }
        },
        //保存temp接口
