    ...
    <el-table :key='tableKey' :data="dataSource" v-loading.body="listLoading" border fit highlight-current-row style="width: 100%">
        ...
        <el-table-column align="center" label="操作类型" min-width="100">
            <template slot-scope="scope">
                ...
                <span v-if="scope.row.type == 5">签收</span>
                <!-- <span v-if="scope.row.type == 5">进货</span> -->
                <!-- <span v-if="scope.row.type == 6">出货</span> -->
                <span v-if="scope.row.type == 101">进货</span>
                <span v-if="scope.row.type == 102">出货</span>
                <span v-if="scope.row.type == 103">库存同步</span>
                <span v-if="scope.row.type == 104">共用库存</span>
            </template>
        </el-table-column>
...
import {findCommodityProductUnitStockRunningWaterOfPage} from '../../../api/getData';
export default {
    data() {
        return {
            ruleForm: {
    ...
    created() {
        this.findCommodityProductUnitStockRunningWaterOfPage();
    },

    methods: {
        //获取列表
        async findCommodityProductUnitStockRunningWaterOfPage() {
            try {
                ...
                const res = await findCommodityProductUnitStockRunningWaterOfPage(obj);
        ...
        handleFilter() {
            ...
            if(this.startEndTime[0]!='' && this.startEndTime[0]!=undefined){
                this.ruleForm.createTimeFront =  this.setTimestamp(this.startEndTime[0]);
            }
            if(this.startEndTime[1]!='' && this.startEndTime[1]!=undefined){
                this.ruleForm.createTimeBack =  this.setTimestamp(this.startEndTime[1]);
            }
            this.findCommodityProductUnitStockRunningWaterOfPage();
        },
        //重置
