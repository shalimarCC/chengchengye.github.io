<template>
    <div id="example">
        <el-steps :space="300" style="" :active="active" finish-status="success" simple>
            <el-step title="选择产品类目"></el-step>
            <el-step title="编辑产品信息"></el-step>
            <el-step title="新建产品完成"></el-step>
        </el-steps>
        <template v-if="active==0">
            <el-cascader
                ...
                filterable
                :props="props"
                style="margin:40px 0;width:400px;"
            ></el-cascader>
            <div><el-button style="margin-top: 12px;" @click='next()'>下一步</el-button></div>
        </template>
        ...
        <template v-else-if="active==1">
            <el-form ref="form" :model="form" :rules="rules" label-width="80px" style="padding:20px 200px;min-width:400px;">
                ...
                <el-form-item label="产品编码" style="text-align:left;">
                    <span style="font-size:16px;line-height:36px;">{{form.showProductAttNameId}}</span>
                </el-form-item>
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="120px" style="padding:20px 200px;min-width:400px;">
                ...
                <el-row :gutter="60">
                    <el-col v-if="form.lists" v-for="(info,index) in form.lists" :key="index">
                        <el-form-item v-if="info.mode == 1" :label="info.name" style="text-align:left;">
                            <el-select v-model="info.value" placeholder="请选择" style="width:100%;" :disabled="info.key==3&&form.lists[1].value!=3" v-if="index!=3" @change="outin">
                                <el-option
                    ...
                    <div class="warn-col" v-if="warnTiping">
                        ...
                        <div class="skuList">
                            <el-table
                                ...
                                min-height="500">
                                    ...
                                    align="center">
                                        <template slot-scope="scope">
                                            <el-button type="primary" size="small" @click="handleCloseValue(scope.row.productAttValueId,scope.row.id,info.id)">移除</el-button>
                                        </template>
                                    </el-table-column>
                                </el-table>
                        </div>
            ...
            <div><el-button style="margin-top: 12px;" @click='next1()' :loading="nextLoading">下一步</el-button></div>
        </template>
...
import {getCategoryList ,idToName,categoryAttNameByCId,getQiNiuToken,deletePicture,updateProduct,findByProductIdAndAttNameId,attributeValueByProductIdAndCategoryId,saveProductAttValueByProductIdAndAttnameId,productAttValueDeleteById,saveImgByProductAttValueId,delImgByProductAttNameId,findCommodityTemplateByType} from '../../../api/getData';
export default {
    data() {
...
　　　　　　 },
    ...
    //   warnTip(){
    //     if(this.form.lists){
    //         if(this.form.lists[0].value == 1 && this.form.lists[1].value ==3){
    //              this.warnTiping = true;
    //              return this.warnTiping;
    //          }
    //     }
    //   }
    },
    methods:{
        inputreg(){
            if(this.form.lists){
                var reg =  /^[0-9]*$/;
                if(!reg.test(this.form.lists[5].value) || !reg.test(this.form.lists[6].value) || !reg.test(this.form.lists[7].value)){
                    this.$message({
                        type: 'error',
                        message: '仅允许输入正整数数字'
                    });
        ...
        //根据类目Id获取类目属性和属性值
        async categoryAttNameByCId () {
            try {
                const res = await categoryAttNameByCId({categoryId:this.form.categoryId});
                if (res.data.code == 0) {
                    var tfis = this;

                    this.form.lists = [];
                    this.form.lists1 = [];
                    $.each(res.data.data.attList,function(key,info){
                    ...
                    console.log(this.form.lists);
                    $.each(res.data.data.parameterAttList,function(key,info){
