    ...
    <div id="example">
        <template v-if="active==1">
                ...
                <el-form-item label="请选择可以兑换的新批次" required>
                    ...
                    <el-table v-if="tableData && tableData.length>0" :data="tableData" border style="width:100%" :show-header="false" class="newTableList">
                        ...
                        <el-table-column width="80">
                            <template slot-scope="scope">
                                加价￥<el-input  @change="inputNum" v-model="scope.row.addPrice" maxlength="30"></el-input>
                            </template>
                        </el-table-column>
                        <el-table-column width="80">
                            <template slot-scope="scope">
                                序号<el-input  @change="inputNum" v-model="scope.row.sort"></el-input>
                            </template>
                        </el-table-column>
                    </el-table>
                </el-form-item>
            ...
            <el-dialog class="forListOut" title="请选择可以兑换的旧批次" :visible.sync="dialogVisible2" width="60%" center>
                    ...
                    <el-table ref="selected" :data="tableForNewDate2" height="500" border highlight-current-row style="width: 100%"  :row-key="getRowKeys2">
                        <el-table-column align="center" width="30">
                            <template slot-scope="scope">
                                <el-checkbox :label="scope.row.id">1</el-checkbox>
                            </template>
                    ...
                    <div class="searchBtns">
                        <el-button type="primary" @click="comfirmOld" :disabled="disable">确认</el-button>
                    </div>
            </el-dialog>
        </template>
        <!-- 新增成功 -->
...
import {addNewActWithTx,findCouponBatchByParam,findExchangeActivity} from '../../../api/getData';
export default{
    data(){
        return{
            getRowKeys(row) {
        ...
        "$route": {
            handler(curVal,oldVal){
                ...
                if(str == "/editToNewAct"){
                    ...
                    this.selectedNew =[];
                    this.selectedOld = [];
                    this.findExchangeActivity();
                    this.findCouponBatchByParam();
                    this.findCouponBatchByParam2();
                    // if(this.selectedNew && this.tableForNewDate){
    ...
    methods:{
        ...
        handleClose(index) {
            let that = this;
            that.form.oldBatchIds = [];
            that.tagListOld.splice(index,1);
            that.dynamicTags.splice(index,1);
            $.each(that.dynamicTags,function(key,info){
                that.form.oldBatchIds.push(info.id);
            })
            console.log(that.form.oldBatchIds);
            that.$message({
                type: 'success',
                message: '删除成功!'
            });
        ...
        async addNewActWithTx() {
            try {
                ...
                obj.exchangeId = this.$route.params.id;
                console.log(this.form.oldBatchIds);
        ...
        handleSelectionChange2(selection){
            ...
            let arrStr = "";
            $.each(this.selectedOld,function(key,info){
                arr.push(info.id);
        ...
        inputNum(){
            ...
            for(var i=0;i<this.tableData.length;i++){
                if(!reg.test(this.tableData[i].addPrice) || !reg.test(this.tableData[i].sort)){
                    this.$message({
                        type: 'error',
        ...
        //编辑查询接口
        async findExchangeActivity() {
            try {
                this.platformId = this.$store.state.Cookie.mutations.fetch().platformId;
                console.log(this.platformId);
                const res = await findExchangeActivity({id:this.$route.params.id,platformId:this.platformId});
                if (res.data.code == 0) {
                    let arr = res.data.data.oldBatchInfo;
                    if(arr){
                        for(var i=0;i<arr.length;i++){
                           this.dynamicTags.push({
                               id:arr[i].id,
                               title:arr[i].title,
                               titleStr:arr[i].detail
                           });
                           this.form.oldBatchIds.push(arr[i].id);
                           this.tagListOld.push(arr[i].id);
                        }
                    };
                    this.form.exchangeName = res.data.data.exchangeName;
                    this.form.endDay = res.data.data.endTime;
                    this.form.unitStatus = JSON.parse(JSON.stringify(res.data.data.status));
                    this.tableData = res.data.data.newBatchInfo;
                    console.log(this.tagListOld);
                    let arrnew = this.tableData;
                    if(arrnew){
                        for(var i=0;i<arrnew.length;i++){
                            this.form.newBatch.push({
                                batchId:arrnew[i].id,
                                addPrice:arrnew[i].addPrice,
                                sort:arrnew[i].sort
                            });
                           this.tagListNew.push(arrnew[i].id);
                        }
                    }
                    console.log('111');
        ...
        async comfirmOld(){
            try {
                ...
                if(this.tagListOld && this.tagListOld.length>0){
                ...
                if (res.data.code == 0) {
                    ...
                    // $.each(this.dynamicTags,function(key,info){
                    //     this.form.oldBatchIds.push(info.id);
                    // })
                    this.form.oldBatchIds = arr;
                    console.log(this.dynamicTags);
                    console.log(this.form.oldBatchIds);
        ...
        async comfirmNew(){
            try {
                ...
                if(this.tagListNew && this.tagListNew.length>0){
