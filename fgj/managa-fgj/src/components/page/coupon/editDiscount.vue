<template>
    <div id="example">
        <template v-if="active==1">
            ...
            <el-dialog title="已选择门店" :visible.sync="dialogFormVisible3" size="tiny">
...
import {findMerchantProductOfPage,findStandardUnitCombinationOfPage,findCompanyIdOrName,queryStandardUnitListByBlurry,getQiNiuToken,
insertOrUpdateCouponWithTx,findCouponById,findStoreAllInfo,findCouponStoreOfPage} from '../../../api/getData';
export default {
    data() {
        return {
            form:{
        ...
        async findCouponById () {
            try {
                ...
                if (res.data.code == 0) {
                    ...
                    if(obj.companyList[0] < 0){
                        this.allStatus = obj.companyList;
                        this.form.companys = [];
                    }else{
                        this.form.companys = obj.companyList;
                    }
                    if(obj.stores[0] < 0){
                        this.allStatus2 = false;
                    }else{
                        this.$refs.tree1.setCheckedKeys(obj.stores);
                    }
                } else {
                    this.$message({
                        type: 'error',
