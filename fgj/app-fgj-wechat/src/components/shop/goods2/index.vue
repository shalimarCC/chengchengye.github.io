<template>
    <div id="goods" v-if="goodType">
        ...
        <div id="goods_bottom" v-show="buyType==1">
            <div id="noGoods" v-if="goodsData.status !=3"><span>商品已经下架了</span></div>
            <div id="pay" :class="{'buy_btn_disabled':goodsData.status !=3}" @click="buy">立即兑换</div>
        </div>
      <!--  <mt-popup
            ...
            <div v-for="(item,type) in couponList" class="couponInfo">
              <div class="couponTop">
                ...
                <div class="couponTime">
                  ...
                  <!-- <div class="logos expired" v-if="item.couponUnitStatus=="></div> -->
                  <!--<div class="logos got" v-if="item.couponBatchStatus==3"></div>-->
                  <div class="logos used" v-if="item.couponBatchStatus==1"></div>

                  <span class="time">
                    <!--{{item.isShowEffectTimeRange?(setTime(item.effectStartTime)-setTime(item.effectEndTime)):item.effectTimeRange}}-->
                    <b>{{item.effectTimeRange}}</b>
                    <!--<div class="get" @click="getCoupon(item.id)">立即领取</div>-->
                    <div class="get" v-if="item.couponBatchStatus==0" @click="getCoupon(item.id)">立即领取</div>
                    <div class="use" v-if="item.couponBatchStatus==2" @click="useCoupon(item.goodsId,item.jumpType,item.couponUnitId,item.effectTimeRange,item.isShowEffectTimeRange)">立即使用</div>
    ...
    export default{
        ...
        methods:{
            ...
            async getCoupon(id){
              ...
              try {
                const res = await insertCouponUnitWithTx({id:id,storeId:'1'});
          ...
          //获取优惠券列表
            ...
            useCoupon(id,jumpType,couponId,effectTimeRange,isShowEffectTimeRange){
