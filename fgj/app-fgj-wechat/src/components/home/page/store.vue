  ...
  export default {
    ...
    methods:{
      async  findPageCfgById(){
        try{
          const res = await findPageCfgById({scope:0,pageType:this.pageType});
          if(res.data.code==0){
            Indicator.close();
            this.showHead = true;
      ...
      //查询外链
      async insurance3rdLogin(id) {
        try {
          const res = await insurance3rdLogin({type:id});
          ...
          case 8:
            if(obj.available) {
              switch (obj.linkId) {
                ...
                //将目标规格加入购物车
                case 2:
                  this.saveCartItem(obj.extParam1,obj.extParam2);
                  break;
                //确认订单
                case 3:
                  this.$router.push('/shop/orderConfim'+'?puId='+obj.extParam1+'&num='+obj.extParam2);
                  break;
                //领取优惠券
                case 4:
