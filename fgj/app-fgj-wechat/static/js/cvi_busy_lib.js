    ...
    if (div.length > 0) {
        ...
        for (i = 0; i < div.length; i++) {
            if (div[i].className == 'buzy_ele') {
                ...
                div[i].firstChild.style.height = div[i].style.height;
                div[i].firstChild.style.width = div[i].style.width;
                break;
            }
        }
    }
...
function onIEVPResize(event) {
    ...
    if (vp) {
        if (typeof document.documentElement != 'undefined') {
            vp.style.width = document.documentElement.clientWidth + 'px';
            vp.style.height = document.documentElement.clientHeight + 'px';
        } else {
            vp.style.width = document.getElementsByTagName('body')[0].clientWidth + 'px';
            vp.style.height = document.getElementsByTagName('body')[0].clientHeight + 'px';
        }
    }
}
...
function onIEVPScroll(event) {
    ...
    if (vp) {
        if (typeof document.documentElement != 'undefined') {
            vp.style.left = document.documentElement.scrollLeft + 'px';
            vp.style.top = document.documentElement.scrollTop + 'px';
        } else {
            vp.style.left = document.getElementsByTagName('body')[0].scrollLeft + 'px';
            vp.style.top = document.getElementsByTagName('body')[0].scrollTop + 'px';
        }
    }
}
    ...
    if ((typeof(parent) === 'object' || parent == 'viewport') && document.getElementsByTagName) {
        ...
        if (parent == 'viewport') {
            ...
            if (!viewport) {
                ...
                cz = viewport.style;
                cz.backgroundColor = 'transparent';
                cz.position = 'fixed';
                cz.overflow = 'hidden';
                cz.display = 'block';
                cz.zIndex = 999999;
        ...
        if (typeof(parent) === 'object') {
            ...
            isVL = document.all && document.namespaces ? 1 : 0;
            ...
            inner.style.cursor = 'progress';
            inner.style.display = 'block';
            inner.style.zIndex = (cz - 1);
            ...
            if (typeof(cs.borderRadius) == "undefined") {
                ...
                if (typeof(cs.WebkitBorderRadius) != "undefined") {
            ...
            if (text != "") {
                ...
                string.style.zIndex = (cz - 0 + 1);
                string.style.top = '50%';
                string.style.left = '0px';
                string.style.marginTop = 2 + (size / 2) + 'px';
                string.style.textAlign = 'center';
                string.style.width = 100 + '%';
                string.style.height = 'auto';
                if (styles != "") { string.innerHTML = '<span ' + (styles.match(/:/i) ? 'style' : 'class') + '="' + styles + '">' + text + '</span>'; } else { string.innerHTML = '<span>' + text + '</span>'; }
            ...
            return {
                remove: function() {
                    if (waiting) {
                        ...
                        if (parent.id == 'viewport') { parent.style.display = 'none'; }
                    }
                },
...
function getBusy(obj, cl, sz, tp, ir, w, ct, sp, mo) {
    ...
    var running = false,
        ...
        al = 0,
        f = 100,
        c, h, p, t, x, y, v, hp, ph, sh, ele = new Array();
    c = getHEX(cl);
    tp = tp || "t";
    ...
    for (i = 0; i < ct; i++) {
        ...
        sh.style.lineHeight = '1px';
        sh.style.fontSize = '0px';
        sh.style.top = y + 'px';
        sh.style.left = x + 'px';
        sh.style.backgroundColor = c;
    ...
    function nextLoop() {
        ...
        if (document.all && !window.opera) {
            for (i = 0; i < ct; i++) {
                al = ((os + i) % ct);
                ele[al].style.filter = "progid:DXImageTransform.Microsoft.Alpha(opacity=" + parseInt(Math.min(1, Math.max(mo, 1 - ((ct + 1 - i) / (ct + 1)))) * 100) + ")";
            }
        } else {
            for (i = 0; i < ct; i++) {
                al = ((os + i) % ct);
                ele[al].style.opacity = Math.min(1, Math.max(mo, 1 - ((ct + 1 - i) / (ct + 1))));
            }
        }
...
function getBusyVL(obj, cl, sz, tp, ir, w, ct, sp, mo) {
    ...
    var running = false,
        ...
        al = 0,
        f = 100,
        c, i, h, p, t, x, y, hs, qs, hw, qw, rp, sh, fl, ele = new Array();
    c = getHEX(cl);
    tp = tp || "t";
    t = (tp.match(/^[coprt]/i) ? tp.substr(0, 1).toLowerCase() : 't');
    ct = Math.max(5, Math.min(36, ct || 12));
